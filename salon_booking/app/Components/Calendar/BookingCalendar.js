...
const BookingCalendar = ({ setBookingDate }) => {
  const now = new Date();
  ...
  return (
    <div className="flex flex-col gap-5 items-center bg-white rounded-[10px] p-8 h-full w-full border border-black">
      {/* Month Header */}
      <div className="w-[98%] relative border-b border-[var(--border-gray)] pb-3" ref={pickerRef}>
        <div className="flex justify-between items-center p-2">
          <button
            ...
            className="p-2 rounded-[10px] text-[10px] text-[#8B8B8B] text-[20px]"
            style={{
          ...
          <div className="flex items-center gap-1 text-black font-pMedium text-[20px]">
          ...
          <button
            ...
            className="p-2 rounded-[10px] text-[10px] text-[#8B8B8B] cursor-pointer"
              ...
              {monthNames.map((m, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={disabled}
                    className={`py-1 rounded font-pMedium text-[20px] ${
                      currentMonth === i
                        ? "bg-emerald-300 font-bold"
                        : disabled
                        ? "text-[#8B8B8B]/[.5] cursor-not-allowed"
      ...
      <div className="grid grid-cols-7 w-[98%] p-2">
        {dayNames.map((d, i) => (
          <div key={i} className="text-center text-gray-500 font-pMedium text-[20px]">
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 w-[98%] gap-2 p-2">
        ...
        {Array.from({ length: adjustedFirstDay }).map((_, i) => {
          ...
          return (
            <div 
                key={`p${i}`} 
                className="text-center text-gray-400 font-pMedium text-[20px]"
                style={{
                    cursor: "not-allowed"
                }}
            >
        ...
        {Array.from({ length: daysInMonth }).map((_, idx) => {
          ...
          return (
            <div
              ...
              className={`text-center py-1 rounded cursor-pointer font-pMedium text-[20px] ${
                disabled
